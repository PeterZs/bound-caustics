...
def readexr(a, crop = {}):
    ...
    return np.minimum(imga, 1e4) * 0.15

figure = CropComparison(
    reference_image=readexr("results/fig_sphere_Ref.exr"),
    method_images=[
        readexr("results/fig_sphere_Bounded.exr"),
        readexr("results/fig_sphere_Enum.exr"),
        readexr("results/fig_sphere_MPG.exr"),
        readexr("results/fig_sphere_SMS.exr"),
        readexr("results/fig_sphere_UPSMCMC.exr"),
    ],
    crops=[
        Cropbox(top=620, left=620, height=60, width=60, scale=5),
        Cropbox(top=654, left=1015, height=60, width=60, scale=5),
    ],
    scene_name="Sphere (40 sec)",
    method_names=["Reference", "Ours",  "SP", "MPG", "SMS", "UPSMCMC"],
    spp_list= [12, 1, 5, 5, 19]
)
figuregen.figure([figure.figure_row], width_cm=19, filename="results/fig_sphere.pdf")
os.system("results\\fig_sphere.pdf")
