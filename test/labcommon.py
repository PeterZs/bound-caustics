...
def run_cmd(TESTID, command, name):
    ...
    t1 = time.time()
    with open('results/{}_log.txt'.format(TESTID + "_" + name), 'w') as file:
        file.write(log_str)
        print("@timeused", t1-t0, file=file)
        # print("@memused", mem_used, file=file)
        file.close()
    with open('results/{}_err.txt'.format(TESTID + "_" + name), 'w') as file:
        file.write(err_str)
        file.close()
...
def run_test(TESTID, name, scene_file, mts_version, params, appendix, overwrite = True):
    ...
    mem.proc_name = "mitsuba.exe" if mts_version == 2 else "Mitsuba.exe"
    cmd += scene_file + " "
    filename = "results/{}.exr".format(name)
